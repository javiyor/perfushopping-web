...
<div class="page">
  <nav aria-label="breadcrumb" class="mb-2">
    <a href="/soluciones" style="color:rgba(246,244,239,0.7)">Soluciones</a>
    <span style="color:rgba(246,244,239,0.5);margin:0 8px">/</span>
    <span><?= htmlspecialchars((string)$need['name']) ?></span>
  </nav>

  <?php if ($need['image']): ?>
  <div style="margin-bottom:16px">
    <img src="<?= htmlspecialchars((string)$need['image']) ?>" alt="" style="width:100%;max-height:320px;object-fit:cover;border-radius:12px">
  </div>
  <?php endif; ?>

  <h2 style="margin:0 0 10px"><?= htmlspecialchars((string)$need['name']) ?></h2>
  <?php if ($need['description_long']): ?>
  <div class="notice"><?= nl2br(htmlspecialchars((string)$need['description_long'])) ?></div>
  <?php elseif ($need['description_short']): ?>
  <p class="lead"><?= htmlspecialchars((string)$need['description_short']) ?></p>
  <?php endif; ?>

  <?php if ($topics): ?>
  <h3 style="margin:24px 0 10px">Temas relacionados</h3>
  <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px">
    ...
    <a href="/temas/<?= htmlspecialchars((string)$t['slug']) ?>" class="card-link" style="text-decoration:none;color:inherit">
      <div class="card" style="padding:12px;text-align:center">
        <?php if ($t['image']): ?><img src="<?= htmlspecialchars((string)$t['image']) ?>" alt="" style="width:100%;height:100px;object-fit:cover;border-radius:6px;margin-bottom:6px"><?php endif; ?>
        <div class="small"><?= htmlspecialchars((string)$t['name']) ?></div>
      </div>
    </a>
    <?php endforeach; ?>
  </div>
  ...
  <div class="mt-3">
    <a class="btn" href="https://wa.me/?text=<?= urlencode('Hola, estoy buscando una solución para ' . ($need['name'] ?? '') . ' en Perfushopping.') ?>" target="_blank" rel="noopener">Consultar por WhatsApp</a>
  </div>
</div>
