...
final class HomeBlockRepo
{
    ...
    public function findForHome(): array
    {
        ...
        $st = Db::pdo()->prepare("
            ...
            WHERE page_type = 'home' AND page_id = 0 AND active = 1
    ...
    {
        $st = Db::pdo()->query('SELECT * FROM cms_page_blocks WHERE page_type = \'home\' AND page_id = 0 ORDER BY position ASC, id ASC');
        $rows = $st->fetchAll() ?: [];
        foreach ($rows as $i => $row) {
            $rows[$i]['settings'] = $this->decodeSettings($row['settings'] ?? '');
        }
    ...
    public function save(array $data): int
    {
        ...
        $st = Db::pdo()->prepare('INSERT INTO cms_page_blocks (page_type, page_id, block_type, position, active, start_at, end_at, title, settings, content, target_entity_type, target_entity_id, segment, tracking_data) VALUES (\'home\', 0, :bt, :pos, :a, :sat, :eat, :t, :set, :c, :tet, :tei, :seg, :td)');
        $st->execute($fields);
